...
export async function GET(request: NextRequest) {
  try {
    ...
    // Pagination support for large datasets (optional - if not provided, returns all contacts)
    ...
    const limit = usePagination ? Math.min(parseInt(paginationLimit) || 1000, 5000) : null; // Max 5000 per page
    const page = usePagination ? Math.max(parseInt(paginationPage) || 1, 1) : null;

    console.log(`[Contacts API] Fetching contacts for user: ${userId}, pageId: ${pageId || 'all'}, fromDatabase: ${fromDatabase}${usePagination ? `, page: ${page}, limit: ${limit}` : ''}`);
    ...
    if (fromDatabase && !pageId) {
      try {
        ...
        // If pagination is requested, use single query with range
        if (usePagination && limit && page) {
          const offset = (page - 1) * limit;
          console.log(`[Contacts API] Using pagination: offset ${offset}, limit ${limit}`);

          const { data: paginatedContacts, error: paginatedError } = await supabaseServer
            .from("contacts")
            .select("*")
            .eq("user_id", userId)
          ...
          if (paginatedError) {
            console.error("[Contacts API] Error loading paginated contacts:", paginatedError);
            return NextResponse.json(
              { error: "Failed to load contacts", details: paginatedError.message },
              { status: 500 }
            );
          }

          // Transform and return paginated results
          const transformedContacts = (paginatedContacts || []).map((c: any) => ({
            id: c.contact_id,
            name: c.contact_name,
            page: c.page_name,
            pageId: c.page_id,
          ...
          console.log(`[Contacts API] Returning ${transformedContacts.length} contacts (page ${page}, total ${totalCount || 0})`);
          ...
          return NextResponse.json({
            ...
            pagination: {
              ...
              hasMore: offset + transformedContacts.length < (totalCount || 0)
            }
          });
        }
